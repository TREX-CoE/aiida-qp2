...
def show_tree(wavefunction, nodes):
    ...
    try:
        ...
        colors = [ Colors.Red if ii % 2 == 0 else Colors.Red for ii in range(ldata)]
...
def show_graph(wavefunction, nodes):
    ...
    if len(missing_modules) > 0:
        echo.echo(f"Please install the following modules to show the graph: {', '.join(missing_modules)}")
        echo.echo("")
        return

    dict_of_nodes = {x.child: x for x in nodes}
    dict_of_nodes[wavefunction.pk] = CalcHolder(wavefunction.pk, None, "root", None)
