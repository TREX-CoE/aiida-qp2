#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Run a 2-step test calculation on tutor.
...
def load_aiida_setup():
    """Load tutor computer and qp2@tutor from the AiiDA database.
    ...
    # Load the tutor computer
    try:
        computer = orm.load_computer('tutor')
    except:
        raise Exception(
            'Create the tutor computer for this example') from NotExistent

    # Create or load the qp2 code
    try:
        code = orm.load_code('qp2@tutor')
    except NotExistent:
...
def test_run_create_ezfio(code, computer):
    ...
    Runs on the tutor computer using qp2@tutor code
    """
    # Set up inputs
    ...
    builder.metadata.options.computer = 'tutor'
...
def test_run_scf_from_ezfio(code, computer, ezfio_RemoteData_inp):
    ...
    qp2_commands = [
        f'set_file {ezfio_inp_name}', 'run scf', 'run save_for_qmcchem'
    ]

    qmcchem_commands = [
        'source /home/evgeny/qmcchem/qmcchemrc',
    ...
    builder_scf.metadata.options.computer = 'tutor'
